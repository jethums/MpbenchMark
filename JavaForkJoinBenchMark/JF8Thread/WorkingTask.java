...
public class WorkingTask extends RecursiveAction {

	private static final long serialVersionUID = 1856930024755418001L;
	private int id = -1;
	...
	double[][] workingArray;
	public WorkingTask(double[][] inputArray){
		workingArray=inputArray;
	}

	@Override
	protected void compute() {
		if(workingArray.length<=configuration_data.workload){
			PerformCalculation();
		}
		else{
			int mid=workingArray.length/2;
			double[][] leftArray=new double[mid][workingArray[0].length];
			double[][] rightArray=new double[workingArray.length-mid][workingArray[0].length];
			for(int i=0;i<mid;i++){
				for(int j=0;j<workingArray[0].length;j++){
					leftArray[i][j]=workingArray[i][j];
					rightArray[i][j]=workingArray[i+mid][j];
				}
			}
			for(int j=0;j<workingArray[0].length;j++){
				rightArray[workingArray.length-mid-1][j]=workingArray[workingArray.length-1][j];
			}
			WorkingTask left=new WorkingTask(leftArray);
			WorkingTask right=new WorkingTask(rightArray);
	...
	private void PerformCalculation(){
		...
		rtPerformancedata.nextNumPoints();
		int CurrentPoint = (int)workingArray[0][4]+1;
